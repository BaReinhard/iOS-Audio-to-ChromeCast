multiRoomApp.controller('HomeController', [
    '$scope',
    'buttonService',
    '$http',
    '$timeout',
    function($scope, buttonService, $http, $timeout) {
        $scope.getButtons = function() {
            buttonService.get().then(function(buttons) {
                $scope.buttons = buttons.data;
                console.warn(buttons.data);
                console.error($scope.buttons);
        ...
        $scope.refreshSinks = function() {
            $http.get('/refreshSinks').then(function(res) {
                console.log(res);
